...
TOTAL_TIMESTEPS = NUM_STEPS * 1000   # Total timesteps to run
            ...
            if (t+1) % NUM_STEPS==0:
                ...
                for ep in range(NUM_EPOCHS):
                    ...
                    for k in range(num_grads):
                        ...
                        advantage_batch = torch.tensor(advantage_batch, dtype=torch.float32)
                        return_batch = torch.tensor(return_batch, dtype=torch.float32)

                        # Update the networks on minibatch of data
                        ret = policy.update(obs_batch, action_batch, log_prob_batch, advantage_batch, return_batch)
