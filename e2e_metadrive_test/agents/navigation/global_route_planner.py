...
class GlobalRoutePlanner(object):
    ...
    def trace_route(self, origin, destination):
        ...
        #
        #plt.ion()
        for k in self._graph.edges():
            edge = self._graph.edges[k[0], k[1]]
            #print(edge['path'])
            if len(edge) >1:
                X = [t.transform.location.x for t in [edge['entry_waypoint']] + edge['path'] + [edge['exit_waypoint']]]
                Y = [t.transform.location.y for t in [edge['entry_waypoint']] + edge['path'] + [edge['exit_waypoint']]]
                plt.plot(X,Y,'k-')
        X = []
        Y = []
        for ele in route_trace:
            X.append(ele[0].transform.location.x)
            Y.append(ele[0].transform.location.y)
        #print('global route waypoints', X, Y)
        plt.plot(X,Y,'ro')
        plt.plot(X[0],Y[0],'b*')
        plt.plot(X[-1],Y[-1],'ko')
        ...
        for p in self._topology:
            X1 = [ p['entryxyz'][0], p['exitxyz'][0] ]
            Y1 = [ p['entryxyz'][1], p['exitxyz'][1] ]
    ...
    def _build_topology(self):
        ...
        for segment in self._wmap.get_topology():
            ...
            #print(segment)
        #print(len((self._topology[5]['path'])))
        #print(len((self._topology[24]['path'])))
        #print()
    ...
    def _build_graph(self):
        ...
        for segment in self._topology:
            ...
            #print(road_id, section_id, lane_id)
    ...
    def _path_search(self, origin, destination):
        ...
        #print(self._wmap.get_waypoint(origin), self._wmap.get_waypoint(destination))
        #print(start, end)
        route = nx.astar_path(
