...
FULL_FRAME_SIZE = (1928, 1208)
W, H = FULL_FRAME_SIZE[0], FULL_FRAME_SIZE[1]
eon_focal_length = FOCAL = 2648.0
...
def get_view_frame_from_road_frame(roll, pitch, yaw, height, offset=0):
  device_from_road = orient.rot_from_euler([roll, pitch, yaw]).dot(np.diag([1, -1, -1]))
  view_from_road = view_frame_from_device_frame.dot(device_from_road)
  return np.hstack((view_from_road, [[offset], [height], [0]]))


# aka 'extrinsic_matrix'
# road : x->forward, y -> left, z->up
def get_view_frame_from_road_frame_1(roll, pitch, yaw, height):
  # TODO
...
#   device_from_road = orient.rot_from_euler([roll, pitch, yaw])
#   view_from_road = view_frame_from_device_frame.dot(device_from_road)
#   return np.hstack((view_from_road, [[0.], [height], [0]]))



def get_view_frame_from_calib_frame(roll, pitch, yaw, height, offset = 0):
  device_from_calib= orient.rot_from_euler([roll, pitch, yaw])
  view_from_calib = view_frame_from_device_frame.dot(device_from_calib)
  return np.hstack((view_from_calib, [[offset], [height], [0]]))
...
def denormalize(img_pts, intrinsics=eon_intrinsics, width=1928, height=1208):
