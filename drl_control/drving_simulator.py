...
class DrivingSimulator:
    ...
    def reset(self):
        ...
        max_ay = 2.0
        # random inputs
        v0 = max(0.5, np.random.rand() * 12)

        v0 = 10.0

        # max_kappa = min(max_ay / ( v0 ** 2), 0.04)
        in_A0 = (np.random.rand() - 0.5) * 1.0
        in_A1 = (np.random.rand() - 0.5) * 0.05
        in_A2 = (np.random.rand() - 0.5) * 0.001 * 0
        in_A3 = (np.random.rand() - 0.5) * 0.00001 * 0

        in_yawrate_degps =  (2 * in_A2 * v0 * 180 / np.pi + np.random.randn() * 0.3)
        ...
        return np.array([v0, self.a0, self.a1, v0, self.a0, self.a1, in_yawrate_degps, 0, 0]).reshape(1, 9)
    ...
    def step(self, action, is_active=True):
        ...
        in_yawrate_degps = psi_rate_degps + 0.2 * np.random.randn()
        next_state = np.array([v_ego, cof[3], cof[2], v_ego, self.cof_last[3], self.cof_last[2], in_yawrate_degps, 0, 0]).reshape(1, 9)
        ...
        state_reward = 0.5 * lat_vel_reward + 1.0 * lat_pos_reward
        reward += state_reward
        ...
        lateral_accel_threshold = 1.0
        lateral_accel_reward = 1.0  - abs(lateral_accel / lateral_accel_threshold)
        reward += 1.0 * survive_reward + lateral_accel_reward * 10.
        ...
        #     reward += 20.
            ...
            mission_status = 0
        return next_state, reward, mission_status
    ...
    def test_model_rl(self):

        model = torch.load(
        './rl_model_baseling.pt', map_location="cuda", weights_only=False).cuda()
        model.eval()
        init_stat = self.reset()
        gt_lat_mpc = LatMpc()
        gt_lat_mpc.prev_time = self.delay_time
        ...
        for t in range(200):
            ...
            ctrl_state.append(init_stat[5])
            tx_in = torch.from_numpy(init_stat).float().cuda()
            pred_out, pred_std, _ = model(tx_in.reshape(1, 9)) # here is radps
            # print(pred_out)
