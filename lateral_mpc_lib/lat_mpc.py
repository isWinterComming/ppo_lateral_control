...
def get_mpc_cffi():
    ...
    '''
    with open('./lateral_mpc_lib/lat_mpc_ert_rtw/lat_mpc.h', "r") as h:
        file_header = h.read()

    # get model funtion
    header_func = re.findall("extern void lat_mpc_.*;", file_header, re.S)
    header_func = '\n'.join(header_func)
    header_func = header_func.split(';',)

    #print(header_func)

    # get initialize and step function
    header_func = header_func[0] +';' + header_func[1] + ';' + header_func[2] +';'
    # print(header_func)


    Def_C = struct_def + header_func
    ffi.cdef(Def_C)
...
class LatMpc():
    def __init__(self):
        ...
        self.k_control = float(100.)
        self.T_IDX = np.linspace(0.05, 1.5, 30)
    ...
    def update(self, active, current_yawrate_degps, v_ego, C0, C1, C2, C3, C4, C5):
        ...
        self.steer_out =  ffw_wheel_angle*self.STEER_RATIO
